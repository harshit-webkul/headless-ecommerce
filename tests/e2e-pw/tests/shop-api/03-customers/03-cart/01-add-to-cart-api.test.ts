...
import { addItemToCartMutation } from "../../../../mutations/shop-mutation/customers/cart-mutation/cart-mutation";

test.describe("Shop: Add Item to Cart via GraphQL API", () => {
    let apiClient = new GraphQLClient();
    let customerApiClient = new GraphQLCustomerClient();

    test("add item to cart without customizable itmes via graphQL shop api", async () => {
        ...
        expect(res.addItemToCart).toBeTruthy();
        expect(res.addItemToCart.success).toBe(true);
        expect(res.addItemToCart.cart.items.length).toBeGreaterThan(0);
    });

    test("add item to cart with customizable itmes via graphQL shop api", async () => {
        const randomSuffix = Date.now();
