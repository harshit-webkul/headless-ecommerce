...
import {
    removeCartIteMutation,
    addItemToCartMutation,
} from "../../../../mutations/shop-mutation/customers/cart-mutation/cart-mutation";
import { GraphQLClient } from "../../../../utils/adminApiClient";
import { createSimpleProductMutation, updateCustomizableProductMutation } from "../../../../mutations/simple-product-api-mutation";

test.describe("Shop: Remove Cart Item via GraphQL API", () => {
    let apiClient = new GraphQLClient();
    let customerApiClient = new GraphQLCustomerClient();

    test("remove cart item via graphQL shop api", async () => {
        ...
        expect(res.removeCartItem).toBeTruthy();
        expect(res.removeCartItem.success).toBe(true);
    });
});
